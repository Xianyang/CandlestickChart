...
dataFile = "M:\\Chester.Luo\\chartData\\charts15min.xlsx"
...
def write1D(fileName, date1d, high1d, low1d):
    ...
    if date >= date1d:
        ...
        ws.cell(row=rowNum+1,column=13).value = 96
        wb.save(fileName)
...
def write15min(fileName, chartLastdate):
...
if __name__ == "__main__":
    ...
    newLastdate = write15min(dataFile, chartLastdate)
    ...
    if not updateFlag:
        ...
        if date1h.hour == newLastdate.hour:
            duration = 4-int(newLastdate.minute/15)
