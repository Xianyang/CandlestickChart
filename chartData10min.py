...
dataFile = "M:\\Chester.Luo\\chartData\\charts10min.xlsx"
...
def write1D(fileName, date1d, high1d, low1d):
    ...
    if date >= date1d:
        ...
        ws.cell(row=rowNum+1,column=13).value = 144
        wb.save(fileName)
...
def write10min(fileName, chartLastdate):
...
if __name__ == "__main__":
    ...
    newLastdate = write10min(dataFile, chartLastdate)
    ...
    if not updateFlag:
        ...
        if date1h.hour == newLastdate.hour:
            duration = 6 - int(newLastdate.minute/10)
            write1H(dataFile, high1h, low1h, duration)
        else:
            writeLog("1-hour matsuba not updated")
    if NOW.hour==7:
